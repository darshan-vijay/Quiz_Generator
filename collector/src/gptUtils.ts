...
export async function filterUsefulLinks(links: { text: string; url: string }[]): Promise<{ text: string; url: string }[]> {
  const input = links.map((l, i) => `${i + 1}. ${l.text} - ${l.url}`).join('\n');
  ...
  const response = await openai.chat.completions.create({
    model: 'gpt-3.5-turbo',
    messages: [
      { role: 'system', content: 'You are a smart link relevance filter for Wikipedia scraping.' },
...
export async function generateSummaryAndTitle(text: string): Promise<{ title: string; summary: string }> {
  ...
  const response = await openai.chat.completions.create({
    model: 'gpt-3.5-turbo',
    messages: [
      { role: 'system', content: 'You are a summarizer for Wikipedia articles.' },
      { role: 'user', content: `${prompt}\n\n${text.slice(0, 12000)}` }
    ],
    temperature: 0.3
  });
...
export async function extractRelevantLinksFromHtml(html: string, baseUrl: string): Promise<{ text: string; url: string }[]> {
    const prompt = `
  ...
  `;
    ...
    const messages: ChatCompletionMessageParam[] = [
      { role: 'system', content: 'You are a helpful web content parser and link extractor.' },
      { role: 'user', content: `${prompt}\n\nHTML:\n${html.slice(0, 10000)}` }
    ];
